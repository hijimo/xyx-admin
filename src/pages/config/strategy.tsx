...
import { configStrategyColumns } from '@/pages/configurify/columns';
import { ConfigTypeEnum } from '@/enum';

const tableColumns = configStrategyColumns;

const type = ConfigTypeEnum.REWARD_STRATEGY;
...
const UserManageIndex: React.FC = () => {
  ...
  const handleRouteAdd = useCallback(() => {
    //
    history.push(`/config/strategy/${type}/add`);
  }, []);
    ...
    return _values(
      produce(tableColumns, (draft) => {
        draft.option!.render = (_, record: DictItemSSD) => {
          return (
            <>
              <Link to={`/config/strategy/${type}/${record.dictCode}/edit`}>编辑</Link>
