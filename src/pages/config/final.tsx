...
import { configRewardColumns } from '@/pages/configurify/columns';
import { ConfigTypeEnum } from '@/enum';

const tableColumns = configRewardColumns;

const type = ConfigTypeEnum.FIANL_REWARD;
...
const UserManageIndex: React.FC = () => {
  ...
  const handleRouteAdd = useCallback(() => {
    //
    history.push(`/config/final/${type}/add`);
  }, []);
    ...
    return _values(
      produce(tableColumns, (draft) => {
        draft.option!.render = (_, record: DictItemSSD) => {
          return (
            <>
              <Link to={`/config/final/${type}/${record.dictCode}/edit`}>编辑</Link>
