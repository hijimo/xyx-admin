...
import type { RouteProductDetailSSD, FreightListItemSSD } from '@/types';
...
const FreightInfo: React.FC<FreightInfoProps> = ({ data, routeId, onSuccess }) => {
  ...
  const changeEnumList = useCallback((list) => {
    return (
        ...
        draft.option!.render = (_, item) => {
          return (
            <>
              ...
              <Link
                to={`/product/routeProduct/${routeId}/offers/${item.id}/expenses/${item.expenseItem}/quotes`}
              >
                报价设置
              </Link>
            </>
  ...
  }, [handleAddOrEdit, handleConfirm, routeId]);
